...
# Task 7B.13: Update API routes to accept an injected store (or store factory) so backends can be swapped without editing route handlers.

Plan item (Phase 3 – Implementation) from `plans/phase-7b-storage-backends-plan.md`: Update API routes to accept an injected store (or store factory) so backends can be swapped without editing route handlers.
