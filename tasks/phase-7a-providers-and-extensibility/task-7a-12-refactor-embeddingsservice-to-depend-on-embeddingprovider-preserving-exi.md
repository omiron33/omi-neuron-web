...
# Task 7A.12: Refactor `EmbeddingsService` to depend on `EmbeddingProvider` (and optional token/cost helpers), preserving existing behavior and batching/rate-limits.

Plan item (Phase 3 – Implementation) from `plans/phase-7a-providers-and-extensibility-plan.md`: Refactor `EmbeddingsService` to depend on `EmbeddingProvider` (and optional token/cost helpers), preserving existing behavior and batching/rate-limits.
